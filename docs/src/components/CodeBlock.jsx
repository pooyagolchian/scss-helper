...
function tokenize(code, lang) {
  ...
  const patterns = {
    ...
    js: [
      { type: 'comment',  re: /^(\/\/[^\n]*|\/\*[\s\S]*?\*\/)/ },
      { type: 'string',   re: /^(["'`][^"'`\n]*["'`])/ },
      { type: 'keyword',  re: /^(import|export|from|const|let|var|function|return|require|module|default|true|false|null)\b/ },
      { type: 'property', re: /^(--[\w-]+)/ },
      { type: 'function', re: /^([\w$]+(?=\())/ },
      { type: 'plain',    re: /^([\w$]+)/ },
      { type: 'punct',    re: /^([{}();:,./=<>!&|+\-*[\]])/ },
  ...
  return (
    <code>
      {tokens.map((t, i) => {
        ...
        return <span key={i} style={{ color }}>{t.value}</span>
      })}
    </code>
  )
}
