      ...
      <Section title="How it works" id="how">
        <Prose>
          <p>
            ...
            <code>@media (prefers-color-scheme: dark)</code> for system-level preference.
            No duplication — a single mixin generates both.
          </p>
        </Prose>
      ...
      <Section title="Toggle with JavaScript" id="js-toggle">
        <Demo label="Click to simulate dark mode toggle">
          <div style={{ display: 'flex', gap: '1rem', alignItems: 'center' }}>
            <button
              style={{ padding: '0.5rem 1rem', borderRadius: '6px', border: '1px solid var(--border)', background: 'var(--bg-card)', color: 'var(--text)', cursor: 'pointer', fontFamily: 'var(--font-mono)', fontSize: '0.8rem' }}
              onClick={() => {
            ...
            <span style={{ color: 'var(--text-muted)', fontSize: '0.8rem' }}>Toggles on &lt;html&gt;</span>
          </div>
        </Demo>
...
root.setAttribute('data-theme', saved)`}
      ...
      <Section title="Dark mode utilities" id="utilities">
        <Prose>
          <p>Utility classes with the <code>.dark:</code> prefix override color in dark mode.</p>
        </Prose>
...
.nav {
  ...
  @include dark-mode {
    ...
    border-bottom: 1px solid var(--card-border, #2d2d2d);
        ...
        <Callout type="tip">
          Define your dark values as CSS custom properties — this lets you override from JavaScript or from the top of your stylesheet without recompiling SCSS.
        </Callout>
      </Section>
    </Page>
  )
