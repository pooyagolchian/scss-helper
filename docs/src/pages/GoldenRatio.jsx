import { Page, PageHeader, Section, SubSection, Prose, Callout, PropTable, Demo } from '../components/Page'
...
function goldenStep(n, base = 1) {
  let r = base
  if (n > 0) { for (let i = 0; i < n; i++) r *= phi }
  else if (n < 0) { for (let i = n; i < 0; i++) r /= phi }
  return r
}
  ...
  return (
    <Page>
      <PageHeader
        ...
        title={<>The Golden Ratio<br /><span style={{ color: 'var(--accent)', fontStyle: 'italic' }}>φ = 1.618…</span></>}
      ...
      <Section title="What is φ?" id="what-is-phi">
        ...
        <Prose>
          <p>
            The key insight: <strong>divide any value by φ (÷ 1.618)</strong> to get the minor portion.
            <strong>Multiply by φ (× 1.618)</strong> to get the next step up.
            Chaining these steps creates a natural, harmonious scale used in everything
            from the Parthenon to Apple product design.
          </p>
        </Prose>
...
font-size: golden-step(3);    // → 4.236rem`}
      ...
      <Section title="Golden Typography Scale" id="typography">
        <Prose>
          <p>Every step is the previous × φ. This produces the most visually harmonious type hierarchy possible — the same proportions used in classical typography.</p>
        </Prose>
          ...
          {typeScale.map(({ step, rem, px, label }) => (
            <div key={step} className={styles.scaleRow}>
              <div className={styles.scaleMeta}>
                <code className={styles.scaleStep}>step {step > 0 ? '+' : ''}{step}</code>
                <span className={styles.scaleLabel}>{label || ''}</span>
                <span className={styles.scaleValues}>{rem}rem / {px}px</span>
              </div>
              <div
...
.gs-leading-compact { line-height: 1.0;   }`}
        ...
        <Callout type="tip">
          <strong>Pro tip:</strong> Use <code>φ (1.618)</code> itself as your body line-height — it is the natural reading rhythm. Use <code>√φ ≈ 1.272</code> for tight headings.
        </Callout>
      </Section>
      ...
      <Section title="Golden Spacing Scale" id="spacing">
        <Prose>
          <p>Nine spacing steps anchored at 1rem, scaling by φ in both directions. Available as CSS custom properties (<code>--gs-space-1</code>…<code>--gs-space-9</code>) and utility classes.</p>
        </Prose>

        <div className={styles.spacingViz}>
          {spacingScale.map(({ step, n, rem, px }) => (
            <div key={step} className={styles.spacingRow}>
              <code className={styles.spacingName}>--gs-space-{step} / .gs-m-{step}</code>
              ...
              <span className={styles.spacingVal}>{rem}rem / {px}px</span>
            </div>
          ))}
        </div>
...
.hero {
  ...
  // → 2.618rem top/bottom, 1.618rem left/right
        ...
        <PropTable rows={[
          { name: '--gs-space-1', type: '0.236rem', default: '~4px',   description: 'φ⁻³ — micro / icon gap' },
          { name: '--gs-space-2', type: '0.382rem', default: '~6px',   description: 'φ⁻² — tight spacing' },
          { name: '--gs-space-3', type: '0.618rem', default: '~10px',  description: 'φ⁻¹ — compact padding' },
          { name: '--gs-space-4', type: '1rem',     default: '16px',   description: 'φ⁰  — base unit' },
          { name: '--gs-space-5', type: '1.618rem', default: '~26px',  description: 'φ¹  — comfortable gap' },
          { name: '--gs-space-6', type: '2.618rem', default: '~42px',  description: 'φ²  — section padding' },
          { name: '--gs-space-7', type: '4.236rem', default: '~68px',  description: 'φ³  — page margin' },
          { name: '--gs-space-8', type: '6.854rem', default: '~110px', description: 'φ⁴  — large whitespace' },
          { name: '--gs-space-9', type: '11.09rem', default: '~177px', description: 'φ⁵  — hero spacing' },
        ]} />
      </Section>
      ...
      <Section title="Golden Ratio Grid" id="grid">
        <Prose>
          <p>
            The classic golden ratio layout: a major column at <strong>61.8%</strong> and minor at <strong>38.2%</strong>.
            Used by countless great publications, landing pages, and editorial designs.
          </p>
        </Prose>

        <Demo label="Golden grid — 61.8% / 38.2%">
          <div style={{ display: 'grid', gridTemplateColumns: '61.8% 38.2%', gap: '1rem', minHeight: '80px' }}>
            <div style={{ background: 'rgba(232,224,200,0.12)', border: '1px solid rgba(232,224,200,0.2)', borderRadius: '6px', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--accent)', fontFamily: 'var(--font-mono)', fontSize: '0.8rem' }}>61.8% — major</div>
            <div style={{ background: 'rgba(232,224,200,0.06)', border: '1px solid var(--border)', borderRadius: '6px', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--text-muted)', fontFamily: 'var(--font-mono)', fontSize: '0.8rem' }}>38.2% — minor</div>
          </div>
        </Demo>
      ...
      <Section title="Golden Aspect Ratios" id="aspect-ratios">
        <Demo label="Golden rectangle, portrait, and landscape">
          <div style={{ display: 'flex', gap: '1rem', flexWrap: 'wrap', alignItems: 'flex-start' }}>
            {[
              { name: '.gs-ratio-golden', ratio: `${phi} / 1`, w: 120, h: 74, label: '1.618:1' },
              { name: '.gs-ratio-golden-portrait', ratio: `1 / ${phi}`, w: 74, h: 120, label: '1:1.618' },
              { name: '.gs-ratio-golden-landscape', ratio: `${phi*phi} / 1`, w: 140, h: 54, label: '2.618:1 φ²' },
            ].map(r => (
              <div key={r.name} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '0.5rem' }}>
                <div style={{ width: r.w, height: r.h, background: 'rgba(232,224,200,0.1)', border: '1px solid rgba(232,224,200,0.3)', borderRadius: '4px', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                  <span style={{ fontSize: '0.7rem', color: 'var(--text-muted)', fontFamily: 'var(--font-mono)' }}>{r.label}</span>
                </div>
...
.gs-rounded-phi-lg { border-radius: 1rem;     }  /* φ⁰  — large panel  */`}
      ...
      <Section title="Mixins & Functions Reference" id="reference">
        <PropTable rows={[
          { name: 'golden-step($n)',             type: 'function', default: 'base: 1rem', description: 'Returns φⁿ rem value. Positive = larger, negative = smaller. Output ∈ {0.236rem → 11.09rem}' },
          { name: 'golden-ratio-split($width)',  type: 'function', default: '',           description: 'Returns 61.8% of $width (major portion)' },
          { name: 'golden-ratio-minor($width)',  type: 'function', default: '',           description: 'Returns 38.2% of $width (minor portion)' },
          { name: '@mixin golden-columns($gap)', type: 'mixin',    default: 'gap: 1rem',  description: 'Two-column grid: 61.8% / 1fr with configurable gap' },
          { name: '@mixin golden-scale-type($step, $leading)', type: 'mixin', default: 'step:0, leading:φ', description: 'Sets font-size at golden step with line-height' },
          { name: '$phi',                         type: 'variable', default: '1.618…',    description: 'Golden ratio constant. Override with !default.' },
          { name: '$phi-minor',                   type: 'variable', default: '0.618…',    description: '1/φ — the minor/smaller portion' },
          { name: '$phi-base',                    type: 'variable', default: '1rem',      description: 'Anchor for the scale. Change to alter all steps.' },
        ]} />
      </Section>
    </Page>
  )
