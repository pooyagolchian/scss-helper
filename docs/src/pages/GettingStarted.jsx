import { Page, PageHeader, Section, SubSection, Prose, Callout } from '../components/Page'
  ...
  return (
    <Page>
      ...
      <Section title="Requirements" id="requirements">
        <Prose>
          <ul>
            <li>Sass <code>≥ 1.60</code> (Dart Sass) — for SCSS source usage</li>
            <li>Node.js <code>≥ 18</code></li>
            <li>Tailwind CSS <code>≥ 3.0</code> — optional, for the plugin</li>
          </ul>
        </Prose>
      </Section>
      ...
      <Section title="SCSS source usage" id="scss-usage">
        <Prose>
          <p>Import the whole library into your main SCSS file. Make sure <code>node_modules</code> is in the Sass load path.</p>
        </Prose>
...
module.exports = {
  ...
  plugins: [
    ...
    scssHelper({ injectTokens: true }),
        ...
        />
        <Callout type="tip">
          <strong>What gets injected?</strong> Design tokens into <code>:root</code>, fluid typography utilities, container query helpers, animation classes, and dark mode token overrides. The Tailwind theme is also extended with <code>colors.primary</code>, <code>colors.danger</code>, etc.
        </Callout>
      </Section>
    </Page>
  )
