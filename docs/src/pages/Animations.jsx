import { Page, PageHeader, Section, Prose, Callout, Demo, PropTable } from '../components/Page'
  ...
  return (
    <Page>
      ...
      <Section title="Animation classes" id="animations">
        <Prose>
          <p>Add <code>.animate-{'{name}'}</code> to any element. Click a card to preview.</p>
        </Prose>

        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(200px, 1fr))', gap: '0.5rem', margin: '1.5rem 0' }}>
          {animations.map(({ name, desc }) => (
            <div
              key={name}
              style={{ padding: '1rem', background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: '8px', cursor: 'pointer' }}
              ...
              <div style={{
                ...
                <div
                  style={{
                    ...
                    animation: active === name ? `${name} 1s ease infinite` : 'none',
                  }}
                />
              </div>
              <code style={{ fontSize: '0.7rem', color: 'var(--accent)', display: 'block', fontFamily: 'var(--font-mono)', marginBottom: '0.25rem' }}>
                .animate-{name}
              </code>
...
<div class="animate-slide-up animate-delay-300">Card 3</div>`}
      ...
      <Section title="Transition utilities" id="transitions">
        <PropTable rows={[
          { name: '.transition',            type: 'all 200ms ease', default: '', description: 'Transition all properties' },
          { name: '.transition-colors',     type: 'color, background-color, border-color', default: '200ms', description: 'Smooth color changes' },
          { name: '.transition-opacity',    type: 'opacity',       default: '200ms', description: 'Fade in/out' },
          { name: '.transition-transform',  type: 'transform',     default: '200ms', description: 'Move/scale/rotate' },
          { name: '.transition-shadow',     type: 'box-shadow',    default: '200ms', description: 'Shadow hover effects' },
          { name: '.duration-75',           type: 'animation-duration', default: '', description: '75ms' },
          { name: '.duration-100',          type: '',              default: '',      description: '100ms' },
          { name: '.duration-200',          type: '',              default: '',      description: '200ms' },
          { name: '.duration-300',          type: '',              default: '',      description: '300ms (default)' },
          { name: '.duration-500',          type: '',              default: '',      description: '500ms' },
          { name: '.ease-linear',           type: 'linear',        default: '',      description: 'Linear easing' },
          { name: '.ease-in',               type: 'ease-in',       default: '',      description: 'Ease in' },
          { name: '.ease-out',              type: 'ease-out',      default: '',      description: 'Ease out (most natural)' },
          { name: '.ease-in-out',           type: 'ease-in-out',   default: '',      description: 'Ease in + out' },
        ]} />
...
:root {
  ...
  --easing: cubic-bezier(0.4, 0, 0.2, 1);
      ...
      <Section title="Reduced motion" id="a11y">
        <Callout type="tip">
          Every <code>.animate-*</code> and <code>.transition-*</code> class is automatically disabled for users who have <strong>prefers-reduced-motion: reduce</strong> set in their OS settings. No extra work required.
        </Callout>
