  ...
  return (
    <Page>
      ...
      <Section title="Colors" id="colors">
        <Prose>
          <p>
            All colors are exposed as CSS custom properties. The Sass <code>$color-*</code>{' '}
            variables are also available for SCSS usage.
          </p>
        </Prose>
          ...
          {colorTokens.map(t => (
            <div key={t.name} className={styles.swatch} style={{ background: t.hex }}>
              ...
              <span
                ...
                style={{
                  color: t.text === 'light' ? 'rgba(255,255,255,0.6)' : 'rgba(0,0,0,0.5)',
                }}
              >
                {t.hex}
              </span>
  ...
  &:hover {
    background: darken($color-primary, 10%);
      ...
      <Section title="Spacing" id="spacing">
        <Prose>
          <p>
            Spacing tokens on a <code>0.25rem</code> (4px) scale. 25 steps from 0 to 96 (0 → 24rem).
          </p>
        </Prose>
...
.section {
  ...
  --line-height-loose:  2;
      ...
      <Section title="Importing the standalone tokens CSS" id="import">
        <Prose>
          <p>
            Use the pre-compiled token bundle without any SCSS tooling — just add it to your HTML
