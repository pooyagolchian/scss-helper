import { Page, PageHeader, Section, SubSection, Prose, Callout, PropTable, Demo, Grid2 } from '../components/Page'
  ...
  return (
    <Page>
      ...
      <Section title="Colors" id="colors">
        <Prose>
          <p>All colors are exposed as CSS custom properties. The Sass <code>$color-*</code> variables are also available for SCSS usage.</p>
        </Prose>

        <div className={styles.colorGrid}>
          {colorTokens.map(t => (
            <div key={t.name} className={styles.swatch} style={{ background: t.hex }}>
              <span className={styles.swatchName} style={{ color: t.text === 'light' ? '#fff' : '#000' }}>
                {t.name}
              </span>
              <span className={styles.swatchHex} style={{ color: t.text === 'light' ? 'rgba(255,255,255,0.6)' : 'rgba(0,0,0,0.5)' }}>
                {t.hex}
              </span>
            </div>
          ))}
  ...
  &:hover {
    background: darken($color-primary, 10%);
      ...
      <Section title="Spacing" id="spacing">
        <Prose>
          <p>Spacing tokens on a <code>0.25rem</code> (4px) scale. 25 steps from 0 to 96 (0 → 24rem).</p>
        </Prose>
...
.section {
        ...
        <div className={styles.typeTable}>
          {fontSizeTokens.map(t => (
            <div key={t.name} className={styles.typeRow}>
              ...
              <span style={{ fontSize: t.val, lineHeight: 1, color: 'var(--text)', fontFamily: 'var(--font-sans)' }}>Ag</span>
              <span className={styles.typeVal}>{t.val} / {t.px}</span>
            </div>
          ))}
        </div>
  ...
  --line-height-loose:  2;
      ...
      <Section title="Importing the standalone tokens CSS" id="import">
        <Prose>
          <p>Use the pre-compiled token bundle without any SCSS tooling — just add it to your HTML before any other stylesheet.</p>
        </Prose>
        <CodeBlock lang="html" code={`<!-- NPM -->
<link rel="stylesheet" href="node_modules/scss-helper/dist/tokens.css">

<!-- Import in CSS -->
@import 'scss-helper/dist/tokens.css';`} />
        <CodeBlock lang="js" filename="In a JS bundler" code={`import 'scss-helper/css/tokens'`} />

        <Callout type="tip">
          The tokens CSS output is only ~2KB — safe to import globally in any project.
        </Callout>
      </Section>
    </Page>
  )
