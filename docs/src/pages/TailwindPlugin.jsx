...
module.exports = {
  ...
  plugins: [
    scssHelper(),
				...
				<Callout type="tip">
					That's it — all tokens, utilities, and theme extensions are registered
					automatically.
				</Callout>
			</Section>
			...
			<Section title="What gets injected" id="what">
				<Prose>
					<p>
						The plugin uses Tailwind's API to inject content into three layers:
					</p>
				</Prose>
  ...
  // 3. theme.extend → Extend Tailwind's own theme
				...
				<PropTable
					rows={[
						{
							name: "addBase :root tokens",
							type: "Layer: base",
							default: "",
							description:
								"All --color-*, --spacing-*, --font-size-* custom properties",
						},
						{
							name: ".text-fluid-*",
							type: "Layer: utilities",
							default: "",
							description: "Fluid type scale xs → 4xl using clamp()",
						},
						{
							name: ".cq, .cq-size",
							type: "Layer: utilities",
							default: "",
							description: "Container query setup classes",
						},
						{
							name: ".transition-*",
							type: "Layer: utilities",
							default: "",
							description: "Transition utilities + duration/easing helpers",
						},
						{
							name: ".animate-*",
							type: "Layer: utilities",
							default: "",
							description: "All 12 animation classes + delay helpers",
						},
						{
							name: ".gs-grid-*",
							type: "Layer: utilities",
							default: "",
							description: "Golden ratio grid layouts",
						},
						{
							name: ".gs-text-*",
							type: "Layer: utilities",
							default: "",
							description: "Golden ratio type scale -2 → 5",
						},
						{
							name: "theme.colors.*",
							type: "theme.extend",
							default: "",
							description:
								"All scss-helper colors available as Tailwind color names",
						},
						{
							name: "theme.fontSize.*",
							type: "theme.extend",
							default: "",
							description: "fluid-xs, fluid-sm, fluid-base, fluid-lg, fluid-xl",
						},
						{
							name: "theme.transitionDuration",
							type: "theme.extend",
							default: "",
							description: "Adds 75, 150 durations",
						},
					]}
				/>
			</Section>
			...
			<Section title="Using tokens in Tailwind classes" id="tokens">
				<Prose>
					<p>
						Since the plugin extends Tailwind's color palette, you can reference
						scss-helper colors with Tailwind's standard syntax:
					</p>
				</Prose>
...
<div class="animate-fade-in animate-delay-200">Staggered entry</div>`}
			...
			<Section title="Overriding tokens" id="override">
				<Prose>
					<p>
						Extend or override the plugin defaults by passing an options object:
					</p>
				</Prose>
  ...
  plugins: [
    scssHelper({
      ...
      transitions: true,
				...
				<Callout type="info">
					The plugin is under active development. Options API may expand in
			...
			<Section title="Tailwind v4" id="v4">
				<Prose>
					<p>
						Tailwind v4 uses a different plugin API based on CSS{" "}
						<code>@plugin</code> syntax. A v4-compatible adapter is planned for
						the next major release of scss-helper. For now, use the SCSS source
						files directly with v4's <code>@source</code> setup.
					</p>
				</Prose>
