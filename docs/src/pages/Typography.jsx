  ...
  return (
    <Page>
      ...
      <Section title="Font Size Helpers" id="font-size">
        <Prose>
          <p>Generated from the <code>$FONTS_MAP</code> Sass map in <code>_variables.scss</code>. Classes follow the <code>.fs-{name}</code> pattern.</p>
        </Prose>
...
<h1 class="fs-3xl">Page title</h1>`}
      ...
      <Section title="Font Size Modifiers" id="modifiers">
        <Prose>
          <p>Named size classes from <code>_modifiers.scss</code>. Use these for semantic intent rather than numeric sizing.</p>
        </Prose>
...
.font-size-xlarge  { font-size: var(--font-size-xl); }`}
      ...
      <Section title="Fluid Type Scale" id="fluid">
        <Prose>
          <p>
            Uses CSS <code>clamp()</code> to scale font size continuously between a minimum viewport width (<code>320px</code>)
            and a maximum (<code>1440px</code>). No media queries needed.
          </p>
        </Prose>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
            {[
              ...
              <div key={cls} style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
                <code style={{ fontSize: '0.7rem', color: 'var(--accent)', fontFamily: 'var(--font-mono)', minWidth: '140px' }}>.{cls}</code>
                <span style={{ color: 'var(--text-muted)', fontSize: '0.7rem', minWidth: '120px', fontFamily: 'var(--font-mono)' }}>{min} → {max}</span>
              </div>
            ))}
          </div>
        </Demo>
...
<p  class="text-fluid-base">Body that grows gently</p>`}
        ...
        <Callout type="tip">
          Combine fluid type with the golden ratio scale: use <code>golden-step()</code> to set min/max values for <code>fluid-type()</code>.
        </Callout>
      </Section>
